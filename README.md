...
| stub + hashing defaults | still the default | CI stays offline, free and credential-free |
...
LangGraph ships exactly three store implementations (`memory`, `postgres`,
`base`) and neither of those is one of them. Once a relational store has to
...
not, and those are the endpoints that make the memory system inspectable and
deletable. AgentCore *Memory* is a more interesting question than AgentCore
*Runtime*: it is the AWS-native service this project implicitly competes with,
which makes it the most relevant benchmark arm still unbuilt.

The password is generated by RDS into Secrets Manager and injected at task start
— it is not in the template, the repository, or any environment file. It reaches
the app as `ENGRAM_POSTGRES_PASSWORD` and the DSN is assembled in `config.py`, so
the secret never sits inside a connection string that something might log.
